...
COLUMN = input("\nPlease select the MAC Address Column to search from the MAC address file: ")
COLUMN = int(COLUMN)
COLUMN = COLUMN - 1

#Ask the user which column is the ports column
PORTS = input("\nPlease select the Ports Column to search from the MAC address file: ")
PORTS = int(PORTS)
PORTS = PORTS - 1
...
for element in mac_list:
    print(element + "    " + port_list[mac_list.index(element)] + "    " + vendor_list[mac_list.index(element)])
