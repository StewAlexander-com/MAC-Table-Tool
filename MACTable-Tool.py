...
#if the number of elements in mac_list is less than or equal to 5o
if len(mac_list) <= 50:
    print("\nHere is the information for the MAC addresses:\n")
    for element in mac_list:
        #for every element in the vendor_list, print the element the corresponding port and vendor name
        columns = Columns([element, port_list[mac_list.index(element)], vendor_list[mac_list.index(element)]], padding= (0,4))
    try:
        print(columns)
        #exception handling for the rich library if unable to render the columns
    except UnicodeEncodeError or UnicodeDecodeError or UnicodeError or width:
        pass
else:
